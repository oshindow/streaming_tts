...
class InjectAttentionSinksMixin:
    ...
    def _inject_pos_shift_attention(cls, model: PreTrainedModel) -> Optional[int]:
        ...
        ATTENTION_FORWARD_MAPPING = {
            ...
            "qwen2": qwen_pos_shift_attention_forward,
    ...
    def _inject_attention_sink_kv_cache(cls, model, **attention_sink_kwargs) -> int:
        ...
        def overwrite_forward(module):
            ...
            def wrapped_forward(self, *args, **kwargs):
                ...
                # print("outputs.past_key_values", len(outputs.past_key_values), outputs.past_key_values)
                outputs.past_key_values = self.attention_sink_kv_cache(outputs.past_key_values)
                print("outputs.attentions", len(outputs.attentions), outputs.attentions[0].shape)
                print("outputs.past_key_values", len(outputs.past_key_values), outputs.past_key_values[0][0].shape, outputs.past_key_values[0][1].shape)
                return outputs
